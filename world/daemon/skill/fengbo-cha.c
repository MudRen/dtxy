...
"$n不顾自身，手中$W连连刺向$N。这围魏救赵之记果然奏效，将$N逼了回去。\n",
        ...
        ([      "name":                 "「出没风波里」",
                "action":
"$N招式忽缓而不疏，意有余而不尽，竟将风波十二叉上半路八招化为一招刺向$n的$l",
                "dodge":                -50,
		"parry":		-50,
                "damage":               60,
                "force":               60,
...
"$n不顾自身，手中$W连连刺向$N。这围魏救赵之记果然奏效，将$N逼了回去。\n",
        ...
        ([      "name":                 "「出没风波里」",
                "action":
"$N手中$w带起滔天巨浪，挟雷霆万钧之势向$n连刺四叉，风波十二叉下半路四招被$N于一瞬间施展出来",
                "dodge":                -60,
		"parry":		-60,
                "damage":               70,
                "force":               70,
...
{
    int i = me->query_temp("HTH_perform");
    if( !me->query_temp("HTH_perform") ) 
    {
    	return action[random(sizeof(action)-2)];
    }
    else
    {
    	return action[i+sizeof(action)-7];
    }
}
