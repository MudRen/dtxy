...
NOR;
	...
	ap = ap * ap * ap /10 ;
	ap = ap + (int)me->query("daoxing")/2;
	dp = 0.0 + target->query("daoxing")/2;
//	if( random(ap + dp) < dp ) success = 0;
//here we compared exp and spells level.
//note: has nothing to do with target's spells level.

	ap2 = 0.0 + (int)me->query_spi();
	dp2 = 0.0 + (int)target->query_spi();
...
//	if( random(ap + dp + 2500*(ap2+dp2)+200*(ap3+dp3)) < (dp+2500*dp2+200*dp3) ) success = 0;
	if( ((ap + dp + 2500*(ap2+dp2)+200*(ap3+dp3))/1000+1)*random(1000)/1000 < (dp+2500*dp2+200*dp3)/1000 ) success = 0;
