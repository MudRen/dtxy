...
string *weapon1_name = ({ HIR"剑气"NOR,HIM"右手剑"NOR,HIG"快剑"NOR}); 
...
*/
        ...
        msg = HIG"突然之间，寒光一闪，$N剑拉弧形，幽蓝剑光直逼向$n,"NOR+xue_name[random(sizeof(xue_name))]+HIG"阵阵森森寒气。\n"NOR;
            ...
            random(me->query("combat_exp")) > random(target->query("combat_exp") /5) ){                     
                ...
                { 
                	damage += random(damage/2);
                        msg += CYN"$n大吃一惊，变招不及，$w"+CYN"已在他的"NOR+xue_name[random(sizeof(xue_name))]+CYN"对穿而出，护身真气被生生刺穿。\n"NOR;
                }
                else
...
me->start_busy(1+random(2));
...
else                if( force_skill=="xuanzong-force" ) force_skill_name = "玄宗心法";
        ...
        if (random(me->query_skill("force")) > random(target->query_skill("force") /4)&&
            random(me->query("combat_exp")) > random(target->query("combat_exp") /5) ){            
                ...
                { 
                	damage += random(damage/2);
                        msg += CYN"只听$n一声惨嚎,$w"+CYN+"从"NOR+xue_name[random(sizeof(xue_name))]+CYN"刺入，$n连运"NOR+HIR+force_skill_name+NOR+CYN"，气血依然无法凝聚。\n"NOR;
                }
                else
                	msg += CYN"只听$n一声惨嚎,$w"+CYN+"从"NOR+xue_name[random(sizeof(xue_name))]+CYN"掠过，$n连运"NOR+HIR+force_skill_name+NOR+CYN"，气血依然无法凝聚。\n"NOR;
                target->receive_damage("kee", damage);
                target->receive_wound("kee", damage/3);
                //p = (int)target->query("kee")*100/(int)target->query("max_kee");
                //msg += "( $n"+eff_status_msg(p)+" )\n";                
...
int perform3(object me, object target)
{
        ...
        if (random(me->query_skill("force")) > random(target->query_skill("force") /4)&&
            random(me->query("combat_exp")) > random(target->query("combat_exp") /4) ){   
                ...
                { 
                	damage += random(damage/2);
                        msg += weapon3_name[random(sizeof(weapon3_name))]+WHT"刺穿$n的"NOR+HIR+force_skill_name+NOR+WHT",喀嚓一声$n的"NOR+HIM"脖子"NOR+WHT"已然绞断，"NOR+xue_name[random(sizeof(xue_name))]+WHT"被对穿而出。\n"NOR;
                }
                else
