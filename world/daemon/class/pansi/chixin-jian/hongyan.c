...
return notify_fail("“红颜白发”只有盘丝洞门人才可以用！\n");
	if( (string)weapon->query("skill_type") != "sword" && !me->is_manmade_npc() )
	...
	if (me->query_skill_mapped("force")!="jiuyin-xinjing")
        ...
        if( time()-(int)me->query_temp("hongyan_end") < 5 )
	...
	if( (target->query("eff_kee")<0 || !living(target)) && !random(5) )  
        {
		str=me->name()+HIM"使出盘丝洞绝学「"+HIR+"红颜"+HIW+"白发"+HIM"」，"+target->name()+HIM"眼见青丝成白发，伤心过度，郁郁而终！";
