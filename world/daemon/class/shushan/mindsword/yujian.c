...
int perform(object me, object target)
{
	...
	if((int)me->query_skill("taoism", 1) < 120
		...
		cd = 7;
        if(time()-me->query_temp("yjwd_end")<cd) return notify_fail("御剑无敌不能用得这么频繁！\n");         
  ...
  for(i=1;i<=skl;i++)
        ...
        msg=weapon->name()+HIB"飞在空中，突然从"+HIR+where[random(sizeof(where))]+HIB+"方射向$n。\n";
...
//else {
     ...
     message_vision(HIR"\n$N伸手一招，$n"HIR"在空中转了弯，准确无误地落入$N掌中！\n"NOR,me,weapon); 
//}
me->start_busy(2);
return 1;
 }
