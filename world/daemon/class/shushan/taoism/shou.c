...
int cast(object me, object target)
{
	...
	if(!sizeof(inv))	{
		...
		filename = file_name(thing);
		if ( thing->is_newitem() || thing->query_autoload() || thing->query("no_get") || filename[0..9] == "/obj/fabao" || filename[0..10] == "/obj/jiasha")
			msg +=HIM"$N妙手空空，抓到$n身上的"+thing->query("unit")+thing->query("name")+"，可是怎么也拽不下来！\n"NOR;
		else {
