...
int perform(object me, object target)
{
        ...
        int cd = 5;
       if (!(wizardp(me))) {
       if( !wizardp(me) && (string)me->query("family/family_name") != "三界散仙" )
                return notify_fail("你的刺客剑法不是亲传，不能使出[图穷匕现]！\n");
       }
        extra = me->query_skill("spysword",1);
        temp_extra=extra;
if(me->query("force") < 1000||me->query("max_force")<1200) return notify_fail("你的状态不好，无法使出[图穷匕现]。\n");
       ...
       if( me->query("lunhui_zhuanshi") > 1 )
		cd = 4;
       if(time()-me->query_temp("bishou_end")<cd) return notify_fail("刺客剑法不能用得这么频繁！\n");         
