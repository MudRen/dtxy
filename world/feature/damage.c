...
void die()
{
   ...
   int exp, pot, score;
    me = this_object();
        me->clear_condition();  // by emotion
        me->set("kee", 0);  // by emotion
        me->set("sen", 0);  // by emotion
        me->set("force", 0);  // by emotion
        me->set("mana", 0);  // by emotion
...
哪里把$N的尸体抓了回来，接着两手一合，硬生生地把$N魂魄逼回身体。\n"NOR,me);
	...
	inv = all_inventory(this_object());
	for(i=0; i<sizeof(inv); i++)   {
		if( inv[i]->query("newitem_owner_id") )   {
			tell_object(this_object(), HIB+"你看到" + inv[i]->query("name") +
			HIB + "破空而去，慢慢消失在天际．．．\n"+NOR);
			destruct( inv[i] );
		}
	...
	if( userp(ob) )
	{
		...
		else
		{
			...
			else
			{
				...
				// commented out by emotion: else if(ob->query("nowf", 1) < 5)
				else
				{
					ob->set_temp("waters",10);
					ob->add("nowf", 1);
