...
int main(object me, string arg)
{
        ...
        object weapon;
        int i, obstacles,dntg;
        float attack_points, dodge_points, parry_points;
...
if (ob->query("couple/name") != 0) line += sprintf(" 婚姻状况：" HIG"伴侣：%-11s"NOR,ob->query("couple/name"));
	...
	dodge_points = COMBAT_D->skill_power(ob, "dodge", SKILL_USAGE_DEFENSE);
       ...
       line += sprintf( " 体格：[%s %4s %s]  ",
        ...
        line+= "                 ";
        if((int)ob->query("g_balance"))
        	line+= "〖"HIY"存金"NOR"〗："+chinese_number((int)ob->query("g_balance"))+"两"HIY"金票"NOR"\n";
        else
        	line+="〖"HIY"存金"NOR"〗："+HIW"没有任何"HIY"金票"NOR+"\n";
        line += sprintf(" 杀死人造人 0级%d名  1级%d名  2级%d名  3级%d名  4级%d名  5级%d名\n", ob->query("rzr_killed_lvl0", 0),
                  ob->query("rzr_killed_lvl1",0),  ob->query("rzr_killed_lvl2",0),  ob->query("rzr_killed_lvl3",0),
                   ob->query("rzr_killed_lvl4",0),  ob->query("rzr_killed_lvl5",0) );
        line += sprintf(" 杀死人造人 6级%d名  7级%d名  8级%d名  9级%d名 10级%d名 11级%d名\n", ob->query("rzr_killed_lvl6", 0),
                  ob->query("rzr_killed_lvl7",0),  ob->query("rzr_killed_lvl8",0),  ob->query("rzr_killed_lvl9",0),
                   ob->query("rzr_killed_lvl10",0),  ob->query("rzr_killed_lvl11",0) );
        line += sprintf(" 杀死人造人12级%d名 13级%d名 14级%d名 15级%d名 16级%d名 17级%d名\n", ob->query("rzr_killed_lvl12", 0),
                  ob->query("rzr_killed_lvl13",0),  ob->query("rzr_killed_lvl14",0),  ob->query("rzr_killed_lvl15",0),
                   ob->query("rzr_killed_lvl16",0),  ob->query("rzr_killed_lvl17",0) );
        line += "\n";
      if(wizardp(this_player()))
	line += sprintf(" 战斗攻击力 " HIW "%s (+%d)" NOR "\t\t战斗防御力 " HIW "%s (+%d)\n" NOR,
	...
	line += sprintf(" 五行攻击: "HIY"金"NOR"〖%3d〗"HIG"木"NOR"〖%3d〗"HIB"水"NOR"〖%3d〗"HIR"火"NOR"〖%3d〗"YELGRN"土"NOR"〖%3d〗\n",
		ob->query_temp("apply/gold_attack"),ob->query_temp("apply/wood_attack"),ob->query_temp("apply/water_attack"),
		ob->query_temp("apply/fire_attack"),ob->query_temp("apply/earth_attack"));
	line += sprintf(" 五行防御: "HIY"金"NOR"〖%3d〗"HIG"木"NOR"〖%3d〗"HIB"水"NOR"〖%3d〗"HIR"火"NOR"〖%3d〗"YELGRN"土"NOR"〖%3d〗\n",
		ob->query_temp("apply/gold_defense"),ob->query_temp("apply/wood_defense"),ob->query_temp("apply/water_defense"),
		ob->query_temp("apply/fire_defense"),ob->query_temp("apply/earth_defense"));
obstacles = ob->query("obstacle/number");
	...
	dntg = ob->query("lunhui_zhuanshi");
	     if (dntg == 0) line += " "RED"转世投胎"NOR"：你还未曾转世投胎\n";
	     else line+= RED" 转世投胎"NOR"：你已经转世"HIM+chinese_number(dntg)+NOR"次。\n";
...
line += YEL"≡≡≡≡≡≡≡≡≡≡ "HIR"大唐西游"HIY"·"HIG"总站"NOR YEL" ≡≡≡≡≡≡≡≡≡≡≡\n"NOR;
        if ( ob->is_manmade_npc() )
             line += sprintf(WHT+" 为求取真经%s已经历了"HIR"%s"NOR"的岁月 \n\n"+NOR,ob==me?"你":ob->name(1),FINGER_D->age_string( time() - (int)ob->query("manmade/birth")));
        else
