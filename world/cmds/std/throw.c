...
int main(object me, string arg)
{
	string item, targ;
	object ob, victim ;
	int success, found, level, damage,damage_bonus, wound, lucky;
	float ap, dp;
	mixed foo;
	string msg;
	...
	if(success) {
		...
		ap=0.0 + me->query("force");
		dp=0.0 + victim->query("force");
		while(dp*random(1000000)/1000000>ap) {
			damage-=damage/3;
			dp=dp/2;
		}
