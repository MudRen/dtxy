...
int give_reward(object me,mapping quest)
{
    ...
    bonus = quest["bonus"];
    bonus += random(bonus);
    me->add("combat_exp", 4*bonus/5 + num * 10);
    me->add("daoxing", bonus + num * 12);
    me->add("potential", bonus/3);
    me->add("faith", 1);
    //tell_object(me,HIW"你的忠诚度升高了。你还得到了"+chinese_number(bonus*3/4)+
    //            "点实战经验和"+chinese_number(bonus/5)+"点潜能。\n"NOR);
    tell_object(me, HIW"你的忠诚度升高了。\n你得到了"+chinese_number(4*bonus/5 + num * 10)
			    +"点武学经验和"+chinese_number(bonus/3)+
			    "点潜能、"+COMBAT_D->chinese_daoxing(bonus + num * 12)+"道行!\n"NOR);
    num++;
    if(num==10){
        num=0;
        //me->add("daoxing",1000);
        //me->add("potential",200);
        //tell_object(me,HIW"你得到了一年道行和"+chinese_number(200)+"点潜能的附加奖励。\n"NOR);
    }             
...
if( time() - me->query("cancel_time") < 30)
    ...
    if( time() - me->query_temp("mp_xunwu_time") < 60 )
    {
         tell_object(me,ob->name()+"门派仓库资源充足，你等一会再来看看吧!\n");
         return 1;
    }
    // 现在先共用一个quest库
         seteuid(getuid());
    ...
    me->set_temp("mp_xunwu_time", time());
    return 1;
}
