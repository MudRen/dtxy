...
write("一共诛杀"+who->query("office_number")+"名逃犯。\n");
if (who->query("degree"))
   	write( "现任官职：" + who->query("degree") + "。\n");
else
	write( "现任官职：无\n");
write ("────────────────────────────────────\n");
...
//  if (nb == 0)
//  {
		if (who->query("obstacle/reward") == 0 && who->query("lunhui_zhuanshi") == 0)
  		base = 0;
  	else
  		base = 3;
		write ("还剩"+(base-who->query("obstacle/rebirth"))+"根救命毫毛，取经后才能用！！！\n");
	write ("────────────────────────────────────\n");
//  }
}
int killtime(object me)
