...
varargs int receive_wound(string type, int damage, object who) {
	int damage1 = damage;
	if (who && userp(who)) {
		damage1 *= (who->query("lunhui_zhuanshi")+1);
		damage1 /= (query("lunhui_zhuanshi")+1);
                  if (damage1>damage) damage1=damage;
		if (wizardp(this_object()) && query("env/combat")=="verbose") {
			tell_object(this_object(),sprintf("old damage:%d;new damage:%d\n",damage,damage1));
		}
	}
	return ::receive_wound(type,damage1,who);
}
