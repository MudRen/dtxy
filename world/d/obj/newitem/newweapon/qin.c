...
string ori_long="一把灌注了神力的剑，只是剑身弯弯曲曲，
  ...
  if (who->is_busy() || who->query_temp("doing_xiudao") ||        who->query_temp("pending/exercising"))
   ...
   if (! present(arg, who)) 
     ...
     if (me->query("anqi/now") + aq->query_amount() > me->query("anqi/max")) 
         ...
         me->set_item_attr("long", me->query("orilong")+"里面已经装了" +chinese_number(me->query("anqi/now"))+aq->query("base_unit")+aq->query("name")+"，想拆掉用uninstall。\n", who);
