...
int valid_leave(object me,string dir) {
  ...
  ap=to_float(host->query("daoxing"))+to_float(spells)*to_float(spells)*to_float(spells)/10;
  ...
  dp=to_float(me->query("daoxing"))+to_float(spells)*to_float(spells)*to_float(spells)/10;
  if ((ap)*random(1000)/1000<(dp)*random(1000)/1000) return 1;
  message_vision(HIR"$N象只没头苍蝇般撞来撞去，可就是找不着北。\n"NOR,me);
  if (userp(me)) me->start_busy(3+random(5));
  else if (!me->is_manmade_npc())
    me->start_busy(2);
  return 0;
}
