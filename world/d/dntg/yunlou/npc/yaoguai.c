...
{       object me=this_object(), env;
        string *dirs;
        int exp, i, lvl, base;//base给怪增加额外armor,damage的参数
	string where, long;
...
void die()
{
	...
	int max_kee, max_sen, ratio, exp_r, pot_r;
		...
		//str=owner_ob->name()+" got "+exp_r+"/"+exp_reward+"武学"+pot_r+"/"+pot_reward+"潜能"+ dx_r +"道行"+" in level "+ owner_ob->query_temp("mieyao/level")+"。\n";
            ...
            // added by emotion
            newitem = NEWITEM->get_item(me, owner_ob);
            if (newitem) {
				string log_msg;
				message_vision("只见从" + me->query("name") + "的身上掉出一件" + newitem->query("name") +
				"<" + newitem->query("id") + ">来\n", me);
				newitem->move(environment(me));
				if (newitem->is_super_stone())
					command("chat 可怜我珍藏多年的五彩魔法石啊~~~~~~");
				log_msg = newitem->query("name") + "(" + newitem->query("id") + ")";
				if (newitem->is_weapon())
					log_msg = log_msg + "[damage:" + newitem->query("weapon_prop/damage") + "]";
				if (newitem->is_armor())
					log_msg = log_msg + "[armor:" + newitem->query("armor_prop/armor") + "]";
				if (newitem->is_weapon() || newitem->is_armor())
					log_msg = log_msg + "[max_level:" + newitem->query("max_level")
						+ "][max_socket:" + newitem->query("sockets/max") + "]";
				log_msg = log_msg + "\n";
				log_file("newitem_drop",log_msg); 
			}
            // end -- emotion
