...
if (wx>=10000) wx=10000;
    ...
    message("chat",HIC+"〖"+HIW+"大唐西游"+HIC+"〗地藏王菩萨(Dizang pusa)："+me->query("name")+ RANK_D->query_respect(me) +"闯过我幽冥地府十八层地狱，寻得龙筋，当真后生可畏！\n"NOR,users());
                 message("chat",HIC+"〖"+HIW+"大唐西游"+HIC+"〗地藏王菩萨(Dizang pusa)："+me->query("name")+"得到奖励：道行"+chinese_number(dx1)+"年，武学"+wx+"点，潜能30000点．\n"NOR,users());
...
void die()
{
	...
	if( objectp(who) && who->query_temp("jingtian/dizang") )
	{
		...
		if( random(who->query("int")) > 10 )
		{
			...
			message("chat",HIC+"\n\n〖"+HIW+"惊天谣传"+HIC+"〗 地藏王菩萨(Dizang pusa)：" + who->name() + "，你莫要猖狂，等着玉帝、佛祖的惩罚吧！\n\n\n"NOR,users());
   			message("chat",HIC+"\n\n〖"+HIW+"惊天谣传"+HIC+"〗冥王(ming wang)：" + who->name() + "杀死地藏，炼化了地府忧愁之气学会了"+HIB"[惊天三式－－"HIB"愁思"HIR"烈杀"HIC"式"HIB"]"+HIC"！\n\n\n"NOR,users());
		}
		else
		{
			who->set("jingtian_cant",1);
			message("chat",HIC+"\n\n〖"+HIW+"惊天谣传"+HIC+"〗 地藏王菩萨(Dizang pusa)：" + who->name() + "，你莫要猖狂，等着玉帝、佛祖的惩罚吧！\n\n\n"NOR,users());
   			message("chat",HIC+"\n\n〖"+HIW+"惊天谣传"+HIC+"〗冥王(ming wang)：" + who->name() + "杀死地藏，可惜不能炼化地府忧愁之气无法领悟"+HIB"[惊天三式]"+HIC"！\n\n\n"NOR,users());
		}
