...
string ask_mieyao()
{
    ...
    if((string)who->query("family/family_name")=="ÄÏº£ÆÕÍÓÉ½") {
      command("say ÃğÑı³ıÄ§£¬ÄËÎÒ·ğÃÅÖĞÈËÖ®±¾·Ö¡£");
	...
	if( objectp(who) && who->query_temp("huohun/zhenshen") )
	{
		...
		if( hong && random(who->query("int")) > 20 - random( who->query("lunhui_zhuanshi") ) )
		{
			who->add("huohun_known",1);
   			message("chat",HIC+"\n\n¡¼"+HIW+"¾ªÌìÒ¥´«"+HIC+"¡½ºìº¢¶ù(honghai er)£º" + who->name() + "ÖúÎÒ¶á»ØÕæÉí£¬ÖÕÓÚÁìÎòµ½ÁË"+HIR"[»ğ»ê]"+HIC"£¡\n\n\n"NOR,users());
		}
		else
		{
			who->set("huohun_cant",1);
   			message("chat",HIC+"\n\n¡¼"+HIW+"¾ªÌìÒ¥´«"+HIC+"¡½ºìº¢¶ù(honghai er)£º" + who->name() + "ËäÈ»³É¹¦ÖúÎÒ¶á»ØÕæÉí£¬È´ÎŞÔµÑ§Ï°"+HIR"[»ğ»ê]"+HIC"ÕæÊÇ¿ÉÏ§£¡\n\n\n"NOR,users());
		}
		if( hong )
		{
			message_vision(HIR"\n\nºìº¢¶ùÑÛ¼ûÊ±»úÒÑµ½£¬»¯×÷Ò»ÍÅºìÎíÏò¿İËÉ½§»ğÔÆ¶´·ÉÈ¥¡£\n"NOR,who);
			destruct(hong);
		}
...
if (wx>=10000) wx=10000;
			...
			&& (me->query_temp("putuo/windwater")=="done"))
	{
       ...
       message("chat",HIC+"¡¼"+HIW+"´óÌÆÎ÷ÓÎ"+HIC+"¡½¹ÛÒôÆĞÈø(Guanyin pusa)£º"+me->query("name")+"ÖúÎÒÊÕÆëÎäÑ§¾«Òª,½ìÊ±Æ¶É®Ò»¶¨ÉÏÁéÌ¨¹ÛÀñ£¡\n"NOR,users());
             message("chat",HIC+"¡¼"+HIW+"´óÌÆÎ÷ÓÎ"+HIC+"¡½¹ÛÒôÆĞÈø(Guanyin pusa)£º"+me->query("name")+"µÃµ½½±Àø£ºµÀĞĞ"+chinese_number(dx1)+"Äê£¬ÎäÑ§"+wx+"µã£¬Ç±ÄÜ30000µã£®\n"NOR,users());
