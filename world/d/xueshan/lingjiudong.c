...
int do_jie()
{
	object me = this_player();
	object jingang;
	if( !me->query_temp("huohun/yuanshen") && !me->query_temp("piaoxue1/jingang") )
		return 0;
	else if( me->query_temp("huohun/shui") || me->query_temp("piaoxue1/foyin") )
	{
		message_vision("\n这道佛印，已经被你揭开了！\n",me);
		return 1;
	}
	else if( me->query_temp("huohun/yuanshen") )
	{
		me->add_temp("huohun/yuanshen",1);
		me->set_temp("huohun/shui",1);
	...
	{
		me->set_temp("piaoxue1/foyin",1);
		message_vision(HIY"\n$N轻轻将佛印揭开，一道光芒飞出，直奔西天大雷音寺而去！\n"NOR,me);
		message_vision(HIY"\n忽然狂风大作，空中走下两位金刚，大声喝道：谁敢私自揭开佛印？！\n",me);
		me->move("/d/xueshan/no4-enter");
		jingang = new("/d/qujing/kusong/npc/jingang");
		jingang->move(environment(me));
