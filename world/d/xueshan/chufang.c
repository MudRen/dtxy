...
int do_add(string arg)
{
        ...
        if( who->query("piaoxue/handu") && item->query("cook/type") == "shui" )
		return notify_fail("这锅是用来煎药的，别什么东西都往里扔。\n");
	if( who->query("piaoxue/handu") == 1 )
	{
		if( item->query("id") == "lan bingkuai" )
		{
			item->set("cook/allow",1);
			item->set("cook/value",8);
			item->set("cook/type","shui");
		}
	}
	if( who->query("piaoxue/handu") == 2 )
	{
		if( item->query("id") == "shuangse bing" )
		{
			item->set("cook/allow",1);
			item->set("cook/value",10);
			item->set("cook/type","shui");
		}
	}
