...
{       int i,lvl;
        ...
        lvl=(who->query_skill("dengxian-dafa",1)-50)/70;
        if( lvl < 1 ) lvl = 0 ;
        //if (lvl>=5) lvl=4;
        //if (who->query_skill("dengxian-dafa",1)>=400 && (!present("busi niao",environment(who))))
        if (lvl >= 5 && (!present("busi niao",environment(who))))
             lvl=5;
        if( lvl > 5 ) lvl =4;
