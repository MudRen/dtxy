...
void create()
{
        set_name(HIC"银索"HIW"金铃"NOR, ({"yinsuo jinling", "whip"}));
        set_weight(5000);
if( clonep() )
...
mixed hit_ob(object me, object victim)
{
    ...
    if((string)me->query("family/family_name") != "盘丝洞")
    	return;
    if( me->query_skill("yinsuo-jinling",1) > 1 )
    damage_bonus = me->query_skill("yinsuo-jinling",1);
    damage_bonus = damage_bonus / 2 + random(damage_bonus)/4;
    victim->receive_damage("kee",damage_bonus, me);
