...
setup();
  ...
  carry_object("/d/obj/weapon/sword/qingfeng")->wield();//装备天魔琴
	...
	if(me->query("family/family_name") != "盘丝洞" )
        ...
        if( me->query("qingyun_known") > 1 )
      	...
      	if( !me->query("qingyun_known") )
      	{
	      	...
	      	command("say 你切附耳过来。\n");
		message_vision("\n姬邑考在$N耳边念出了一段口诀。\n",me);
		return ("当年四大天王阻挠紫霞仙子下凡，差点误了我们盘丝洞的大事，你去把他们杀了，同时好好参悟我传给你的心法！");
	}
	else//第二段
	{
		if( me->query_skill("jiuyin-xinjing",1) < 300 
			...
			return ("你修为不够，还不能完全掌握琴音清韵的威力！");
		if( me->query_temp("kill/erlang") )
			return ("赶紧去杀掉二郎神！");
		me->set_temp("kill/erlang",1);
		command("say 你切附耳过来。\n");
		message_vision("\n姬邑考在$N耳边念出了一段咒语。\n",me);
		return ("当初捣乱的还有二郎神，此仇不报非君子。不过二郎神修为高深，我再传你琴音扰敌之术，与二郎神对战之时，需好好领悟！");
	}
}