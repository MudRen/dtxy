...
int do_add(string arg) {
  ...
  if (flower->query("id")!="flower" && flower->query("id")!="qiong cao" ) return notify_fail("你不能往百花酿里丢这样东西！\n"); 
  if( flower->query("id") == "qiong cao" && me->query_temp("jueqing/baihua") == 1 )
  {
  	message_vision(HIG"$N将一颗"+flower->query("name")+HIG"溶入百花酿中，花香酒香混作一处，满室芬芳，说不出的甜美。\n"NOR,me);
     	set("baihua",flower->query("leaf"));
     	destruct(flower);
     	me->set_temp("jueqing/baihua",2);
     	return 1;
  }
