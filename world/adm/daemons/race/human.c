...
// begins: emotion
void update_player_health(object ob) {
	mapping my;
	int mycon, mystr, myspi, myint, qi, shen;
	my = ob->query_entire_dbase();
	/* mycon = ob->query_con();
	if ( mycon == 0 ) */
	    mycon = my["con"];
	    // rzr treated differently // emotion
...
/*	myspi = ob->query_spi();
	...
	if ( myint == 0 ) */
	    myint = my["int"];
			...
			qi= to_int((mycon+mystr) * 3 / 2);
			shen= to_int((myspi+myint) * 3 / 2);

		if( userp(ob) || undefinedp(my["max_gin"]) ) {
			if( my["age"] <= 14 ) my["max_gin"] = 100;
			else if( my["age"] <= 30 ) my["max_gin"] = 100 + (my["age"] - 14) * myspi;
			else if( my["age"] <= 80 ) my["max_gin"] = 100 + 16 * myspi;
			else my["max_gin"] = 100 + 16 * myspi  - (my["age"] - 50) * 5;
...
emotion */
    ...
    update_player_health(ob);  // emotion
