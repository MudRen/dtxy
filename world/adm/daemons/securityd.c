...
private mapping trusted_write = ([/*可以写*/
     ...
     "jjhome":   ({"(admin)","(arch)"}),
     "d":   ({ "(wizard)", "(admin)","(arch)" }),
     "obj":   ({ "(wizard)", "(admin)","(arch)" }),
     "data/topten":   ({ "(wizard)", "(admin)","(arch)" }),
     "lestat":   ({ "(wizard)", "(admin)","(arch)" }),
     "u":   ({ "(admin)" }),
//     "cmds/std":    ({ "(wizard)" }),
//     "cmds/usr":    ({ "(wizard)" }),
     "daemon":    ({ "(admin)","(wizard)",}),
...
private mapping exclude_write = ([/*不能写*/
   "jjhome":     ({ "(wizard)","(apprentice)","(immortal)", }),
...
int valid_write(string file, mixed user, string func)
{
   ...
   if( func=="save_object" ) {
     if( (sscanf(base_name(user), "/obj/%*s") ||
         ...
         sscanf(base_name(user), "/jjhome/%*s"))
     &&   sscanf(file, "/data/%*s")
     &&   ( file == (string)user->query_save_file() ||
               file == (string)user->query_save_file()+".o" ||
