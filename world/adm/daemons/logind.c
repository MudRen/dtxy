...
#undef GB_AND_BIG5  // disable BIG5 for Linux, by emotion
...
void calculate_total_reg_users()
{
	int i,j,reg_cnt;
	...
	total_reg_user_counter = reg_cnt;
	remove_call_out("calculate_total_reg_users");
        call_out("calculate_total_reg_users", 1800);
}
...
#ifdef GB_AND_BIG5
    write("            大唐西游欢迎您来访！使用国标码的玩家请键入：gb\n");
...
private void encoding(string arg, object ob)
{
	...
	string ip_name, ip_number, online_num, site_name;
 	int ii;
        mixed info;
	int encode;
...
write(HIR"您的地址在本 MUD 之信誉被人破坏了。\n请E-MAIL:torrow0812@21cn.com申述。\n"NOR);
	...
	site_name = read_file(__DIR__"site_name");
	if(ppl_cnt > max_online)
	   {
	   	online_num = sprintf("%d",ppl_cnt);
	   	write_file(__DIR__"maxonline",online_num,1);
	   }
printf("大唐西游("HIY"YWX人造人"NOR")("HIY + site_name + NOR")现有注册玩家"HIR"%s"NOR"位，共"HIW"%s"NOR"人次访问。\n",chinese_number(total_reg_user_counter),chinese_number(do_counter()));
printf("大唐西游("HIY"YWX人造人"NOR")("HIY + site_name + NOR")开站以来，最高在线数目："HIY"%s"NOR" 人，平均在线数目："HIY"%s"NOR" 人。\n",chinese_number(max_online),chinese_number(avg_online));
	printf("目前共有"HIR"%s"NOR"位管理人员、"HIY"%s"NOR"个人造人、"HIC"%s"NOR"位玩家，以及"HIG"%s"NOR"位使用者在尝试连线。\n\n",
		chinese_number(wiz_cnt), chinese_number(sizeof(children(MANMADE_NPC))), chinese_number(ppl_cnt), chinese_number(login_cnt) );
...
private void get_passwd(string pass, object ob)
{
	...
	if(time()-(int)ob->query("kickout_time") < 600)
			{
			write("你现在没有进入〖大唐西游〗的权利，请十分钟后再联线。\n");
			destruct(ob);
			return;
			}
	// end
...
请您给自己想一个符合〖大唐西游〗神话世界的中文名字。这
个名字将代表你在大唐西游中的人物，而且往后将不能再更改。
...
varargs void enter_world(object ob, object user, int silent)
{
	...
	ob->save();
		...
		tell_room(startroom, user->query("name") + "连线进入这个世界。\n",
			...
			write(CYN"     ≡≡≡≡≡≡≡≡≡≡≡≡"HIR"大唐西游连线通知"NOR CYN"≡≡≡≡≡≡≡≡≡≡≡≡\n");
