// spell_d.c.
// emotion 03/06/2006: fixed possible overflow issue
// mon 12/6/98
//lestat 2001 9重新修正
...
int attacking_cast_success(object attacker, object target, int success_adj)
{
	...
	int xk;
	float ap, dp;
	ap = 0.0 + attacker->query_skill("spells");
        ap = ( ap * ap / 10000 * ap  ); //年做单位进行比较判断.//再/10，才是年 kugu
	//道行超过5000年,提高命中.目前达到这个条件太简单了 kugu
	//ap = ap*(1+attacker->query("daoxing")/5000000);//5万年道行导致ap*11
	ap = ap+attacker->query("daoxing")/1000;
        dp = 0.0 + target->query("daoxing")/1000;//dp/1000保证双方同步 kugu
	success = 0;
	...
	//if( random(100)>dp/(ap+dp+1)*100 ) success = 1; //ap已经相当于dp的2倍
	if( 1+random(ap) > dp ) success = 1;
	...
	int damage, string damage_type, string msg_hit)
{
	...
	if( victim->query_temp("anti_magic") )
	{
		...
		if( success )
		{
			...
			if (wizardp(victim) && victim->query("env/combat")=="verbose") {
				tell_object(victim, sprintf(HIY+"spell ori damage=%d,adjust=%d\n" NOR, damage,adjust/2));
			}
			damage=damage - adjust/2;
			if ( damage>0)
