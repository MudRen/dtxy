...
int quest_reward(object me, object who,object quest_item)
{
        ...
        if(who->query("id")!= quest_item->query("owner_id")) return 0;
        	...
        	tell_object(me,"Äã¶¼ÕâÃ´¸ßÁË£¬Áôµã»ú»á¸øÐÂÈË°É£¡\n");
        	return 1;
        }
...
//            chinese_number(pot) + "µãÇ±ÄÜ\n");
  tell_object(me, sprintf("Äã»ñµÃÁË"HIR"%s"NOR"µÄµÀÐÐ ,"HIY"%s"NOR"µãÇ±ÄÜ ,"HIW"%s"NOR"µãÎäÑ§£¡\n"NOR, daoxing(exp),chinese_number(pot), chinese_number(wx)));
CHANNEL_D->do_channel(this_object(),"rumor",sprintf(HIG+me->name()+NOR"½«Ñ°µ½µÄ"HIW+quest_item->name()+NOR"ËÍ¸ø"+who->name()+"»ñµÃ"HIR+daoxing(exp)+NOR"µÄµÀÐÐ£¬"HIY+chinese_number(pot)+NOR"µãÇ±ÄÜ£¬"
	...
	log_file("static/TASK","["+ctime(time())+"] "
	    ...
	    me->add("balance",1000000);
            tell_object(me,"ÄãÕæÐÐ£¬ÄãµÃµ½ÁË"+chinese_number(999) + "µãÇ±ÄÜºÍÒ»°ÙÁ½»Æ½ðµÄ¸½¼Ó½±Àø£¡\n");
        }
...
string dyn_quest_list()
{
   ...
   output="";
        ...
        output ="[1;32m¡ù[33m´óÌÆÎ÷ÓÎ[31mÊ¹Ãü°ñ[32m¡ù[m\n"+
                "©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥\n"+output+
                "©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥©¥\n\n";
