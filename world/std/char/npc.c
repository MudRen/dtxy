...
int random_move()
{
	...
	object env;  // emotion: bug fix
	env = environment();  // emotion: bug fix
        if (!env) return 0;  // emotion: bug fix
...
{
	string spell_skill;
	if (this_object()->query_temp("in_qingwang")==1)
	{
		message_vision( MAG + query("name") + "眼前出现那个熟悉的身影，被搅的心烦气燥，似离情，剪不断，理还乱。\n"
		                + "竟在生死之间将所学咒语忘得干干净净。\n"NOR, this_object() );
		return;
	}
