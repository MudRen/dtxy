...
object findbox()
{
    if(shop_box && objectp(shop_box) && environment(shop_box)==this_object())
      return shop_box;
...
string value_string(int value)
{
	...
	coin = value % 100;
	if ( gold )
		msg += chinese_number(gold) + "Á½»Æ½ğ";
	if ( silver )
		msg += chinese_number(silver) + "Á½°×Òø";
	if ( coin )
		msg += chinese_number(coin) + "ÎÄÇ®";
	return msg;
}
...
int do_value(string arg)
{
   ...
   if( !value) printf("%sÒ»ÎÄ²»Öµ¡£\n", ob->query("name"));
     ...
     value_string(value / 4), value_string(value / 2));
...
int do_pawn(string arg)
{
   ...
   stamp_ob->set_temp("file", ob);
     stamp_ob->set_temp("value", value/4);
   ...
   this_player()->start_busy(1);
     pay_player(this_player(), value / 4);
   ...
   message_vision("$N°ÑÉíÉÏµÄ" + ob->query("name") + "Âôµô¡£\n",
     this_player());
        ...
        index=ob->query("name")+"("+ob->query("id")+")";
   		...
   		if( ob->is_new_item() || !findbox()->query_temp("goods/"+index) ){
			if (sizeof(sold_items) >= MAX_SOLD_ITEMS) {
				destruct(findbox()->query_temp("goods/" + sold_items[0]));
				findbox()->delete_temp("goods/" + sold_items[0]);
				findbox()->delete_temp("numbers/" + sold_items[0]);
				sold_items = sold_items[1..(sizeof(sold_items) - 1)];
			}
   	        findbox()->set_temp("goods/"+index, ob);
   		   findbox()->set_temp("numbers/"+index, 1);
		   sold_items = sold_items + ({index});
   		   ob->move(findbox());
   		}
   		else {
   ...
   while(i--) {
     ...
     if( j == order )   {
       ...
       arg = obj_file->query("name")+
        ...
        obj_file->query("unit") + obj_file->query("name") + "¡£\n", this_player() );
