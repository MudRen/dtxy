...
void create()
{
        ...
        set("long", @LONG
他就是百世修行的唐僧了。
曾经历尽千辛万苦去西天求取真经，你可以问他有关"转世绝学"的消息。
LONG);
        set("daoxing",1000000);
...
string ask_for_zspfm() {
	...
	if (me->query("lunhui_zhuanshi") < 5)
		return HIY"五转以上才能领悟我的绝学!\n"NOR;
	if (me->query("zspfm/zspfm_ts_number")*20000000 > (me->query("combat_exp")+me->query("daoxing")))
		return HIY"你的修为还不够，领悟不到绝学!\n"NOR;
	me->add("zspfm/zspfm_ts_number",1);
	if (random(me->query("int")) > 45) {
		me->set("zspfm/zspfm_ts",1);
		me->add("zspfm/number",1);
		message( "chat", HIY"\n【转世绝学】：恭喜"+me->query("name")+"在第"+chinese_number(me->query("zspfm/zspfm_ts_number"))+"次的时候领悟了功德佛的绝学！\n"NOR, users() );
